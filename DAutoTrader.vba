...
Function closeYestodayOrder()
	...
	With rs
		if .EOF then
			logString("[ 错误 ]:昨天的开盘表中没有数据，造成无法从数据库中读取代码来进行平仓")
			logString("[ 警告 ]:尝试中交易账户中读取订单数据来进行平仓")
		...
		While (Not .EOF)
			...
			if Cond = "000" then
				logString("[ 错误 ]:该股票" & Label & "的开仓记录和数据库种Condition记录不匹配[这个不应该发生]")
			else
...
Function closeConnection()
	if IsEmpty(DBConnection) then
		logString("[ 提示: ] 数据库没有开，之前可能没有正确关闭")
	else
		DBConnection.Close
		logString("[ 提示: ] 停止运行，成功关闭数据库")
	end if
End Function
